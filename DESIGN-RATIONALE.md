...
**Stance — neuroscience-as-hypothesis, ablation-as-arbiter:** the citation motivates the design; it does **not** justify shipping it. EVB flips ON only if a *discriminating* ablation gate beats `legacy`; otherwise the flag stays OFF and the null result is reported honestly (ablation-or-revert).

**EARNED ON (T2, 2026-06-04).** The original criterion — "beats on **both** promotion precision and high-value retention" — was **non-discriminating**: on the fixed population both metrics saturate at **1.0 in both arms** (the legacy additive blend keeps every genuine high-value memory too), so the gate could never resolve EVB's actual effect (this is the `INCONCLUSIVE` the post-1.0 ledger flagged for W2). The corrected, *discriminating* gate (`evals/evb_gate.py`) decides on **retained-set purity** (`retention_precision = retained ∩ high_value / retained`) under a **no-high-value-regression guard** — the axis EVB is mechanistically designed to move. Result: **EVB `retention_precision` 1.0 vs legacy 0.33** (legacy retains 6 high-value + 6 high-need/zero-gain distractors + 6 unscored-old that survive on the 0.5 neutral-outcome term; EVB's multiplicative `gain×need` zeroes the worthless and keeps only the 6 genuine) with **high-value retention tied at 1.0** (no recall cost) and `distractor_eviction` 1.0 vs 0.0. `evb_enabled` default flipped **ON**; full suite green with the flip (no production coupling broke). This is a genuine purity win, not a metric re-pick — the original criterion was structurally incapable of discriminating.
